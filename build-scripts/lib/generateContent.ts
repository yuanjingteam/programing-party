...
async function shouldRegenerateFiles(
  ...
  try {
    const generatorStats = await fs.stat(generatorFilePath);
    for (const filePath of generatedFilePaths) {
      const fileStats = await fs.stat(filePath);
      if (fileStats.mtime < generatorStats.mtime) {
        return true;
      }
    }
  } catch {
      ...
      await Promise.all(
        frameworkIds.map(async (frameworkId: string) => {
          const frameworkSnippet: FrameworkSnippet = {
            ...
            playgroundURL: "",
            markdownFiles: [],
            snippetEditHref: `https://github.com/yuanjingteam/programing-party/tree/main/content/${sectionDirName}/${snippetDirName}/${frameworkId}`,
          };
...
const TITLE_MAP: Record<string, string> = {
  ...
  "async basics": "异步入门",
  "timeout cancel": "超时与取消",
  "errors basics": "错误处理基础",
  "retry backoff": "重试与指数退避",
};

function dirNameToTitle(dirName: string): string {
  const englishTitle = dirName.split("-").splice(1).join(" ");
  return TITLE_MAP[englishTitle] || capitalize(englishTitle);
}
