...
function generateFrameworkCombinations(): string[] {
  ...
  // Add main page
  combinations.push("https://traeprograming-partydev-mainu7i9.vercel.app/");

  // Add individual framework pages
  for (const framework of frameworks) {
    combinations.push(
      `https://traeprograming-partydev-mainu7i9.vercel.app/?f=${framework.id}`,
    );
  }
  ...
  for (let i = 0; i < popularFrameworks.length; i++) {
    for (let j = i + 1; j < popularFrameworks.length; j++) {
      combinations.push(
        `https://traeprograming-partydev-mainu7i9.vercel.app/?f=${popularFrameworks[i]},${popularFrameworks[j]}`,
      );
    }
  }
  ...
  for (const combo of threeFrameworkCombos) {
    combinations.push(
      `https://traeprograming-partydev-mainu7i9.vercel.app/?f=${combo.join(",")}`,
    );
  }
  ...
  for (const [v1, v2] of versionComparisons) {
    combinations.push(
      `https://traeprograming-partydev-mainu7i9.vercel.app/?f=${v1},${v2}`,
    );
  }
